...
type Config struct {
	...
	Config struct {
		...
		Swapoutms  int      `yaml:"swapoutms"`
		Tracker    struct {
			Name   string `yaml:"name"`
			Config struct {
				Pagesinregion     int `yaml:"pagesinregion"`
				Maxcountperregion int `yaml:"maxcountperregion"`
...
func (p *plugin) StartContainer(pod *api.PodSandbox, ctr *api.Container) error {
	...
	data, err := ioutil.ReadFile("memtierd-config.yaml")
	if err != nil {
		fmt.Println(err)
	}

	var config MemtierdYamlConfig
	...
	config.Policy.Config.Intervalms = 10000
	config.Policy.Config.Cgroups = []string{"/sys/fs/cgroup/test.kubepods"}
	config.Policy.Config.Swapoutms = 10000
	config.Policy.Config.Tracker.Name = "idlepage"
	config.Policy.Config.Tracker.Config.Pagesinregion = 512
	config.Policy.Config.Tracker.Config.Maxcountperregion = 1
	config.Policy.Config.Tracker.Config.Scanintervalms = 10000
	config.Policy.Config.Mover.Intervalms = 20
	config.Policy.Config.Mover.Bandwidth = 100
	...
	err = ioutil.WriteFile("memtierd-config.yaml", newData, 0644)
	if err != nil {
		fmt.Println(err)
	}

	fmt.Println("The YAML file has been modified")

	cat, err := exec.Command("cat", "memtierd-config.yaml").Output()
	if err != nil {
		fmt.Println(err)
	}
	log.Infof("output of cat: %s", cat)
